...
public class Invoice {
    ...
    private List<InvoiceDetail> invoiceDetailList; //发票详情


    public InvoiceVO convert2VO() {
        InvoiceVO invoiceVO = new InvoiceVO();
        invoiceVO.setMsgCode(this.msgCode);
        invoiceVO.setMsg(this.msg);
        invoiceVO.setCode(this.code);
        invoiceVO.setNumber(this.number);
        invoiceVO.setDate(this.date);
        invoiceVO.setChecksum(this.checksum);
        invoiceVO.setTotalAmount(this.totalAmount.toString());
        invoiceVO.setTotalAmountString(this.totalAmountString);
        invoiceVO.setTaxAmount(this.taxAmount.toString());
        invoiceVO.setAmount(this.amount.toString());
        invoiceVO.setBuyerName(this.buyerName);
        invoiceVO.setBuyerCode(this.buyerCode);
        invoiceVO.setBuyerAddress(this.buyerAddress);
        invoiceVO.setBuyerAccount(this.buyerAccount);
        invoiceVO.setSellerName(this.sellerName);
        invoiceVO.setSellerCode(this.sellerCode);
        invoiceVO.setSellerAddress(this.sellerAddress);
        invoiceVO.setSellerAccount(this.sellerAccount);
        invoiceVO.setType(this.type);

        List<InvoiceDetailVO> detailList = new ArrayList<>();
        for (int i = 0; i < this.invoiceDetailList.size(); i++) {
            InvoiceDetailVO detailVO = this.invoiceDetailList.get(i).convert();
            detailVO.setIndex(i + 1);
    ...
    public void setDetailList(List<InvoiceDetail> invoiceDetailList) {
    ...
    public String toString() {
        return "Invoice [msgCode=" + msgCode + ", msg=" + msg + ", code=" + code + ", number=" + number
                ...
                + ", type=" + type + ", detailList=" + invoiceDetailList + "]";
    }
}
