...
public class PdfRegularInvoiceService {
    ...
    public static Invoice getRegularInvoice(String fullText, String allText, int pageWidth, PDDocument doc, PDPage firstPage ) throws IOException {
        ...
        {
            String reg = "发票代码:(?<code>\\d{12})|发票号码:(?<number>\\d{8})|:(?<date>\\d{4}年\\d{2}月\\d{2}日)"
...
//                    invoice.setTitle(m01.group("p").replaceAll("(?:国|统|一|发|票|监|制)", "") + "用发票");
        ...
        {
            String buyer = lab.dragon.invoice.utils.StringUtils.replace(stripper.getTextForRegion("buyer"));
            Pattern pattern = Pattern.compile(reg);
        ...
        {
            String seller = lab.dragon.invoice.utils.StringUtils.replace(stripper.getTextForRegion("seller"));
            Pattern pattern = Pattern.compile(reg);
